...
from Dataset_MM import get_pam_data, get_p12_data, get_p19_data, get_mimic3_data
...
def train_epoch(model, training_data, optimizer, pred_loss_func, opt, classifier):
    ...
    acc, precision, recall, f1, auroc, auprc = 0,0,0,0,0,0
    ...
    for train_batch in tqdm(training_data, mininterval=2,
        ...
        if opt.task == "wbm" or opt.task == "mimic3_phe":
    ...
    if len(sup_preds) > 0:
        ...
        if opt.task == 'wbm' or opt.task == 'mimic3_phe':
            acc, precision, recall, f1, auroc, auprc = evaluate_ml(sup_labels, sup_preds)
        else:
            acc, precision, recall, f1, auroc, auprc = evaluate_mc(sup_labels, sup_preds, opt.n_classes)

    return acc, precision, recall, f1, auroc, auprc, train_loss
...
def eval_epoch(model, validation_data, pred_loss_func, opt, classifier, save_res=False):
    ...
    return acc, precision, recall, f1, auroc, auprc, valid_loss
...
def train(model, training_data, validation_data, testing_data, optimizer, scheduler, pred_loss_func, opt, \
    ...
    if not opt.test_only:
        ...
        for epoch_i in range(opt.epoch):
            ...
            start = time.time()
            train_acc, train_precision, train_recall, train_f1, train_auroc, train_auprc, train_loss = train_epoch(model, training_data, optimizer, pred_loss_func, opt, classifier)
            log_info(opt, 'Train', epoch, train_acc, train_precision, train_recall, train_f1, start=start, auroc=train_auroc, auprc=train_auprc, loss=train_loss, save=True)

            if not opt.retrain:
                start = time.time()
                valid_acc, valid_precision, valid_recall, valid_f1, valid_auroc, valid_auprc, valid_loss = eval_epoch(model, validation_data, pred_loss_func, opt, classifier)
                log_info(opt, 'Valid', epoch, valid_acc, valid_precision, valid_recall, valid_f1, start=start, auroc=valid_auroc, auprc=valid_auprc, loss=valid_loss, save=True)
                ...
                start = time.time()
                test_acc, test_precision, test_recall, test_f1, test_auroc, test_auprc, _ = eval_epoch(model, testing_data, pred_loss_func, opt, classifier, save_res=True)

                log_info(opt, 'Testing', epoch, test_acc, test_precision, test_recall, test_f1, start=start, auroc=test_auroc, auprc=test_auprc, save=True)
    ...
    if not opt.retrain and save_path is not None:
        ...
        start = time.time()
        test_acc, test_precision, test_recall, test_f1, auroc, auprc, _ = eval_epoch(model, testing_data, pred_loss_func, opt, classifier, save_res=True)
        ...
        log_info(opt, 'Testing', best_epoch, test_acc, test_precision, test_recall, test_f1, start=start, auroc=auroc, auprc=auprc, save=True)
...
def main():
    ...
    if opt.wandb:
        wandb.init(
            ...
            project="Warpformer",
            config=vars(opt),
            name=f"{opt.task}-d{opt.d_model}-h{opt.n_head}-l{opt.n_layers}-seed{opt.seed}",
        )
    ...
    if opt.task == "mor":
        ...
        trainloader, validloader, testloader, opt.num_types = get_pam_data(opt, opt.device)
        opt.n_classes = 8  # classification
        opt.median_len = 600

    elif opt.task == 'p12':
        trainloader, validloader, testloader, opt.num_types = get_p12_data(opt, opt.device)
        opt.n_classes = 2  # Binary classification
        opt.median_len = 50  # You may want to adjust this based on your data

    elif opt.task == 'p19':
        trainloader, validloader, testloader, opt.num_types = get_p19_data(opt, opt.device)
        opt.n_classes = 2  # Binary classification
        opt.median_len = 50  # You may want to adjust this based on your data
    elif opt.task == 'mimic3_ihm':
        trainloader, validloader, testloader, opt.num_types = get_mimic3_data(opt, opt.device)
        opt.n_classes = 2  # Binary classification for IHM
        opt.median_len = 75
    elif opt.task == 'mimic3_phe':
        trainloader, validloader, testloader, opt.num_types = get_mimic3_data(opt, opt.device)
        opt.n_classes = 25  # Multi-label classification for PHE (25 phenotypes)
        opt.median_len = 37
