...
class grafiti_(nn.Module):
    ...
    def get_contextualized_embeddings(
        ...
        target_mask: torch.Tensor, # This will be a dummy for classification but needed for structure
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Computes intermediate contextualized embeddings from the GraFITi layers.

        Args:
            time_points: Combined observed and target time points; Tensor (B, T_total)
            values: Observed values; Tensor (B, T_total, D) (target part will be dummy/masked)
            obs_mask: Mask for observed values; Tensor (B, T_total, D)
            target_mask: Mask for target values (structural for classification); Tensor (B, T_total, D)

        Returns:
            edge_emb (Tensor): Edge embeddings (B, K', M)
            t_gathered (Tensor): Time embeddings gathered for edges (B, K', M)
            c_gathered (Tensor): Channel embeddings gathered for edges (B, K', M)
            mask_f (Tensor): Mask for the flattened valid entries (B, K')
        """
        b, _, d = values.shape
        t = time_points.unsqueeze(-1)  # (B, T_total, 1)
        c_onehot = self._one_hot_channels(b, d, device=t.device)  # (B, D, D)

        t_inds, c_inds = self._build_indices(
            time_points, d, t.device
        )  # t_inds (B, T_total, D), c_inds (B, T_total, D)

        # The combined mask indicates all points that have *some* data or are designated as targets
        # For classification, target_mask might be all False for the "target" part of T_total,
        # but obs_mask covers the actual observations.
        # The key is that `mask` defines the universe of points to be flattened.
        mask = obs_mask + target_mask # mask is (B, T_total, D)
        mask_bool = mask.bool()
        ...
        t_inds_f, obs_vals, tgt_mask_f, c_inds_f, mask_f = (
            flattened  # all are of shape (B, K'); K' = total flattened points
        )

        # target_indicator: For forecasting, this helps distinguish observed from queried points.
        # For pure classification using these embeddings, this specific construction might be less critical
        # if we only use obs_mask related embeddings, but we keep it for structural integrity with existing methods.
        # If target_mask_f is all zeros (as it might be if T_target part is empty),
        # then target_indicator will be 1 for observed points (mask_f=1, tgt_mask_f=0)
        # and 0 for padded points (mask_f=0).
        ...
        for i in range(self.n_layers):
            t_gathered_loop = gather(t_emb, t_inds_f)
            c_gathered_loop = gather(c_emb, c_inds_f)

            c_emb = self.channel_time_attn[i](
                c_emb, torch.cat([t_gathered_loop, edge_emb], -1), c_mask
            )
            t_emb = self.time_channel_attn[i](
                t_emb, torch.cat([c_gathered_loop, edge_emb], -1), t_mask
            )

            edge_update = torch.cat(
                [edge_emb, t_gathered_loop, c_gathered_loop], dim=-1
            )
            edge_emb = (
        ...
        return edge_emb, t_gathered, c_gathered, mask_f
    ...
    def forward(
        ...
        for i in range(self.n_layers):
            t_gathered_loop = gather(t_emb, t_inds_f)  # (B, K', M)

            c_gathered_loop = gather(c_emb, c_inds_f)  # (B, K', M)
