...
class AttentionPooling(nn.Module):
    def __init__(self, d_model):
        ...
        self.att = nn.Sequential(
            nn.Linear(d_model, 1),
            nn.Softmax(dim=1)  # over N
        )

    def forward(self, x):  # x: (B, N, D)
        weights = self.att(x)        # (B, N, 1)
        weighted_sum = torch.sum(x * weights, dim=1)  # (B, D)
        return weighted_sum
...
class tPatchGNN(nn.Module):
	def __init__(self, args, supports = None, dropout = 0):
		...
		if hasattr(args, 'num_classes'):
			...
			raise ValueError("num_classes not provided in args")
		...
		self.att_pool = AttentionPooling(enc_dim)
		...
		self.classifier = nn.Sequential(
			...
			nn.Linear(args.hid_dim, args.hid_dim),
			nn.ReLU(inplace=True),
			nn.Linear(args.hid_dim, self.num_classes)
		)
	...
	def classify(self, X, truth_time_steps, mask=None):
		"""
		X (B, M, L, N) 
		...
		mask = mask.permute(0, 3, 1, 2).reshape(-1, L_in, 1) if mask is not None else torch.ones_like(X)  # (B*N*M, L, 1)
		
		# Use provided timestamps for time encoding
		...
		# Global average pooling over variables dimension
		h_pooled = self.att_pool(h) # (B, hid_dim)
		
		# Apply classifier head
		class_logits = self.classifier(h_pooled)  # (B, num_classes)
		
		return class_logits # (B, num_classes)
