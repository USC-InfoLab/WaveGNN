...
def load_and_preprocess_p12():
    """
    Loads and preprocesses the P12 dataset, returning train/val/test splits as lists of tuples:
    (record_id, timestamp, irregular_timeseries, irregular_timeseries_mask, label)
    """
    # Config values from the script header
    ...
    split_num = 1  # You can change this if you want a different split
    ...
    # Normalize using the whole dataset
    ...
    def make_list(indices):
        ...
        for i in indices:
            record_id = int(i)
            ts = timestamps[record_id].squeeze(-1)  # Convert from [length, 1] to [length,]
