...
from lib.MIMIC3_loader import load_and_preprocess_mimic3
from lib.PAM_loader import load_and_preprocess_pam
from lib.P12_loader import load_and_preprocess_p12
from lib.P19_loader import load_and_preprocess_p19
...
def parse_datasets(args, patch_ts=False, length_stat=False):
	...
	### PhysioNet dataset ### 
	### MIMIC dataset ###
	if dataset_name in ["physionet", "mimic", "MIMIC3-IHM", "MIMIC3-PHE", "PAM", "P12", "P19"]:
		...
		elif dataset_name == "MIMIC3-IHM":
			train_data, val_data, test_data = load_and_preprocess_mimic3(file_path="/storage/datasets_public/irreg_ts/datasets/mimiciii/processed/multimodal_processed/ihm")
			seen_data = train_data + val_data
		elif dataset_name == "MIMIC3-PHE":
			train_data, val_data, test_data = load_and_preprocess_mimic3(file_path="/storage/datasets_public/irreg_ts/datasets/mimiciii/processed/multimodal_processed/pheno")
			seen_data = train_data + val_data
		elif dataset_name == "PAM":
			train_data, val_data, test_data = load_and_preprocess_pam()
			seen_data = train_data + val_data
		...
		record_id, tt, vals, mask, label = train_data[0]
		...
		test_dataloader = DataLoader(test_data, batch_size = batch_size, shuffle=False, 
			collate_fn= lambda batch: collate_fn(batch, args, device, data_type = "test",
				data_min = data_min, data_max = data_max, time_max = time_max))
		# check a sample from train_dataloader
		for i, batch in enumerate(train_dataloader):
			if i == 0:
				for j in batch:
					print(j, batch[j].shape if isinstance(batch[j], torch.Tensor) else batch[j])
				break
