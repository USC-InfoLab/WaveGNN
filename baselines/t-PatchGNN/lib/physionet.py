...
def get_data_min_max(records, device):
	...
	for b, (record_id, tt, vals, mask, label) in enumerate(records):
...
def patch_variable_time_collate_fn(batch, args, device = torch.device("cpu"), data_type = "train", 
	...
	Expects a batch of time series data in the form of (record_id, tt, vals, mask) where
		...
		- label is a single value containing the label for the patient.
	Returns:
	Data form as input:
	...
	batch = [(record_id, tt.to(device), vals.to(device), mask.to(device), label.to(device)) for record_id, tt, vals, mask, label in batch]
	...
	predicted_data = []
	predicted_mask = [] 
	for b, (record_id, tt, vals, mask, label) in enumerate(batch):
	...
	if (args.dataset != 'ushcn') and args.dataset != "PAM":
